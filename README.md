...
specify which release will be downloaded, so that you can inspect what the
typesetting steps are doing.
...
information to assemble a *probabilistic* metaweb, *i.e.* in which every
interaction is represented as a single, independent, Bernoulli event of
probability $p$.
